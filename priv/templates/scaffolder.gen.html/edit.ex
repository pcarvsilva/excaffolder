defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>Live.Edit do
 ...
 def render(assigns) do
  <%= inspect Module.concat(schema.web_namespace, schema.alias)%>View.render("edit.html", assigns)
 end
...
def handle_event("save",value, socket) do
  <%= inspect context.module %>.update_<%= schema.singular %>(socket.assigns.<%= schema.singular %>,socket.assigns.changeset.changes)
  send(socket.parent_pid, {"close-modal", value})
  {:noreply, socket}
end
 ...
 def mount(session, socket) do
  {:ok, assign(socket,
              ...
              <%= schema.singular %>: session.<%= schema.singular %>,
